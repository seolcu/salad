...
float get_temperature()
{
     ...
     int counter = 0;
     int j = 0, i;
     float temperature = -1; // DHT11 온습도센서가 측정할 온도 변수, 초기화는 -1로 하여 오류 시 반환할 값으로 설정해놓는다.
     ...
     else // 체크섬 계산 결과와 일치하지 않거나 읽은 데이터가 정상적이지 않은 경우, 이전 측정값으로 반환함.
     {
          // printf("Data not good, reusing previous value...\n");
          temperature = dht_val[2] + dht_val[3] * 0.1;
          printf("Error Temperature : %1.f\n", temperature);
          return temperature; 
     }
...
float get_temperature_five_times(){
     for(int i=0;i<5;i++){ //센서 측정 초기의 불안정한 값 버리기 (5회)
          get_temperature();
     }
     
     float temp;
     float sum = 0;
     for(int i=0;i<5;i++){ //온도를 5번 측정하고, 그것들의 합을 구함
          temp = get_temperature();
          sum += temp;
     }
     return sum /= 5;   //값들의 평균값을 구한다.
}
